 ...
 *
 */
class WeatherLoc(private val mContext: Context, private val mUnitType: TemperatureUnitType? = TemperatureUnitType.STANDARD) {
     ...
     *
     * object which is received in callback is of type [CurrentWeatherData] and it's parameters are as follows.
     *
     *   weather:
     *       coord -
     ...
     *
     * failureBlock -  A method callback that provides the exception which has occurred.
     */
    ...
    /**
     * Below function fetches the weather data for future time based on the latitude, longitude and the
     ...
     * lng - Double datatype value of Longitude.
     *
     * dayCount - Int datatype value of days indicating the numbers of day you want to know the weather of.
     *            this number must be between 1 to 16.
     *
     * successBlock - A method callback that provides the end result of weather data. weather data
     * object has multiple properties through which data can be obtained.
     *
    ...
    fun obtainFutureWeatherByLatLng(
        ...
        lng: Double,
        dayCount: Int,
        successBlock: (weather: FutureWeatherData) -> Unit,
        failureBlock: (exception: Exception) -> Unit
    ) {
        if (isInternetAvailable()) {
            if(dayCount < 17 && dayCount > 0) {
                mOpenWeatherLoc.obtainFutureWeatherFromLatLng(lifecycleOwner, lat, lng, dayCount, successBlock, failureBlock, mUnitType)
            } else {
                val exception: Exception = Exception("Please enter day's count from 1 to 16.")
                failureBlock.invoke(exception)
            }
        } else {
            val exception: Exception = Exception("Internet is not available!")
    ...
    /**
     * Below function fetches the weather data for 5 days from current based on the latitude and longitude.
     *
     * The inner implementation is based on the coroutine hence can be called from any view classes
     * without putting burden to the main thread.
     *
    ...
    fun obtainFiveDaysWeatherByLatLng(
        lifecycleOwner: LifecycleOwner,
        lat: Double,
        lng: Double,
        successBlock: (weather: FutureWeatherData) -> Unit,
        ...
        if (isInternetAvailable()) {
            mOpenWeatherLoc.obtainFiveDaysWeatherFromLatLng(lifecycleOwner, lat, lng, successBlock, failureBlock, mUnitType)
        } else {
            val exception: Exception = Exception("Internet is not available!")
            failureBlock.invoke(exception)
        }
    ...
    companion object {
        ...
        /**
         * The method takes up a double value of temperature in kelvin unit and converts the
        ...
        fun convertKelvinToCelsius(value: Double?): Double {
            ...
            return if (value == null) 0.0 else value - 273.15
        }
        ...
        fun convertKelvinToFahrenheit(value: Double?): Double {
            return if (value == null) 0.0 else ((convertKelvinToCelsius(value)) * 9.0) / 5.0 + 32
        }

        /**
         * The method takes up a double value of temperature in celsius unit and converts the
        ...
        fun convertCelsiusToFahrenheit(value: Double?): Double {
            return if (value == null) 0.0 else value * 9 / 5 + 32
        }

        /**
         * The method takes up a double value of temperature in fahrenheit unit and converts the
        ...
        fun convertFahrenheitToKelvin(value: Double?): Double {
            return if (value == null) 0.0 else convertFahrenheitToCelsius(value) + 273.15
        }
        ...
        fun convertFahrenheitToCelsius(value: Double?): Double {
            return if (value == null) 0.0 else (value - 32) * 5 / 9
        }
